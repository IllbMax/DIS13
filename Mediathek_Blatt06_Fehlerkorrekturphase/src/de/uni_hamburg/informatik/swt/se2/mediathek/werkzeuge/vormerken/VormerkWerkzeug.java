...
import de.uni_hamburg.informatik.swt.se2.mediathek.werkzeuge.SubWerkzeugBeobachter;
...
/**
 * Ein {@link VormerkWerkzeug} stellt die Funktionalität des Vormerkens für die
 * Benutzungsoberfläche bereit. Die UI wird durch die {@link VormerkUI}
 * gestaltet.
 * 
 * @author SE2-Team
 * @version SoSe 2012
...
public class VormerkWerkzeug
{
    ...
    /**
     * Initialisiert ein neues {@link VormerkWerkzeug}. Es wird die
     * Benutzungsoberfläche mit den Ausleihaktionen erzeugt, Beobachter an den
    ...
    {
        _verleihService.registriereBeobachter(new ServiceBeobachter()
        {
    ...
    private boolean istVormerkenMoeglich()
    {
        ...
        boolean vormerkenMoeglich = (kunde != null) && !medien.isEmpty()
                && _vormerkService.istFuerAlleVormerkenMoeglich(kunde, medien)&&hatKeinesGeliehen(kunde,medien);

        return vormerkenMoeglich;
    }
    
    private boolean hatKeinesGeliehen(Kunde kunde, List<Medium> medien)
    {
    ...
    /**
     * Merkt die ausgewählten Medien für einen Kungen vor. Diese Methode wird
